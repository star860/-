...
class AggregateAxletreeData:
    ...
    def get_openstringinfo(self, station_start, station_end):
        """
        从hive数据库取得开行详细信息
        :return:
        """
        sql = '''   SELECT
                    ...
                    WHERE
                        ...
                        AND TIME < '2018-08-02' 
                        AND op_day = '20190901'
                    '''.format(station_start, station_end)
    ...
    def aggregate_data(self):
        ...
        for train_line in train_line_tuples:
            ...
            # 从hive数据库取得开行详细信息
            openstringinfo_tuples = self.get_openstringinfo(station_start, station_end)
            ...
            for one in openstringinfo_tuples:
                ...
                for rowkey_00, column_00 in ts:
                    ...
                    if 'F_DATA:TRAIN_TYPE' in column_00:
                        ...
                        # train_outer_tem = column_00['F_DATA:OUTER_TEM']
                        real_time = re.search(r'_(\d{14})', rowkey_00).group(1)
                        # 获取列车速度的data_code
                        speed_data_code = self.get_data_code(train_type, '00', 1)[1]
                        train_speed = column_00[('F_DATA:{}'.format(speed_data_code))]
                        # 获取列车里程的data_code
                        mileage_data_code = self.get_data_code(train_type, '00', 2)[1]
                        train_mileage = column_00['F_DATA:{}'.format(mileage_data_code)]
                        # 获取列车GPS信息
                        # longitude_data_code = self.get_data_code(train_type, '00', 4)[1]
                        # train_longitude = column_00['F_DATA:{}'.format(longitude_data_code)]
                        # latitude_data_code = self.get_data_code(train_type, '00', 5)[1]
                        # train_latitude = column_00['F_DATA:{}'.format(latitude_data_code)]
                        train_longitude = '00'
                        train_latitude = '00'
                        # 获取该'00'车wtd设备对应的车厢号
                        for coach in ['01', '02', '03', '04']:
                            # 转换成对应车厢的rowkey
                            ...
                            axletree_temperature1 = column_coach[('F_DATA:{}'.format(axletree_temperature1_data_code))]
